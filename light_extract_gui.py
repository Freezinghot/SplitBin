...
class SpectralDataExtractor:
    def __init__(self, root):
        ...
        self.tree = ttk.Treeview(tree_frame, columns=("brightness", "mean_value"), show="headings")
        self.tree.heading("brightness", text="亮度")
        self.tree.heading("mean_value", text="均值 (Mean_Value)")
        self.tree.column("brightness", width=150, anchor="center")
        ...
        # ---------- 图像显示区域 ----------
        plot_frame = tk.LabelFrame(root, text="亮度线性度曲线", padx=5, pady=5)
    ...
    def extract_data(self):
        ...
        for filename in csv_files:
            # 提取亮度值：匹配 "亮度" 后紧跟的数字
            match = re.search(r'亮度(\d+)', filename)
            if not match:
                continue
            brightness = match.group(1)  # 数字字符串
            ...
            results.append((brightness, mean_val))

        if not results:
            self.status_var.set("没有找到包含 '亮度' 信息的 CSV 文件")
            return
        ...
        self.data = results[:]
        for bri, val in results:
            self.tree.insert("", tk.END, values=(bri, val))
    ...
    def export_csv(self):
        ...
        try:
            with open(file_path, 'w', newline='', encoding='utf-8-sig') as f:
                writer = csv.writer(f)
                writer.writerow(["亮度", "均值 (Mean_Value)"])
                for bri, val in self.data:
                    writer.writerow([bri, val])
    ...
    def plot_curve(self):
        ...
        brightnesses = []
        mean_values = []
        for bri_str, mean_str in self.data:
            try:
                b = float(bri_str)
                m = float(mean_str)
        ...
        self.ax.clear()
        self.ax.plot(brightnesses, mean_values, marker='o', linestyle='-', color='b', linewidth=2, markersize=6)
        self.ax.set_xlabel("亮度")
        self.ax.set_ylabel("均值 (Mean_Value)")
        self.ax.set_title("亮度线性度曲线")
        self.ax.grid(True, linestyle='--', alpha=0.6)

        if len(brightnesses) > 1:
            xmin, xmax = min(brightnesses), max(brightnesses)
            xrange = xmax - xmin
            if xrange == 0:
                xrange = 1
            self.ax.set_xlim(xmin - 0.05*xrange, xmax + 0.05*xrange)
        else:
            self.ax.set_xlim(brightnesses[0]-1, brightnesses[0]+1)

        self.canvas.draw()
        self.status_var.set("亮度线性度曲线已更新")
