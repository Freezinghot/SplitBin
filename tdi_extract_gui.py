...
class IntegrationLevelExtractor:
    def __init__(self, root):
        self.root = root
        root.title("积分级数线性度数据提取器")
        ...
        self.tree = ttk.Treeview(tree_frame, columns=("level", "mean_value"), show="headings")
        self.tree.heading("level", text="积分级数")
        ...
        # ---------- 图像显示区域 ----------
        plot_frame = tk.LabelFrame(root, text="积分级数线性度曲线", padx=5, pady=5)
    ...
    def extract_data(self):
        ...
        for filename in csv_files:
            # 提取TDI后的数字：匹配 "TDI" 后紧跟的数字
            match = re.search(r'TDI(\d+)', filename, re.IGNORECASE)
            if not match:
                continue
            level = match.group(1)  # 数字字符串
            ...
            results.append((level, mean_val))

        if not results:
            self.status_var.set("没有找到包含 'TDI' 信息的 CSV 文件")
            return
        ...
        self.data = results[:]
        for lev, val in results:
            self.tree.insert("", tk.END, values=(lev, val))
    ...
    def export_csv(self):
        ...
        try:
            with open(file_path, 'w', newline='', encoding='utf-8-sig') as f:
                writer = csv.writer(f)
                writer.writerow(["积分级数", "均值 (Mean_Value)"])
                for lev, val in self.data:
                    writer.writerow([lev, val])
    ...
    def plot_curve(self):
        ...
        levels = []
        mean_values = []
        for lev_str, mean_str in self.data:
            try:
                l = float(lev_str)
                m = float(mean_str)
                levels.append(l)
        ...
        self.ax.clear()
        self.ax.plot(levels, mean_values, marker='o', linestyle='-', color='b', linewidth=2, markersize=6)
        self.ax.set_xlabel("积分级数")
        self.ax.set_ylabel("均值 (Mean_Value)")
        self.ax.set_title("积分级数线性度曲线")
        self.ax.grid(True, linestyle='--', alpha=0.6)

        if len(levels) > 1:
            xmin, xmax = min(levels), max(levels)
            xrange = xmax - xmin
            if xrange == 0:
                xrange = 1
            self.ax.set_xlim(xmin - 0.05*xrange, xmax + 0.05*xrange)
        else:
            self.ax.set_xlim(levels[0]-1, levels[0]+1)

        self.canvas.draw()
        self.status_var.set("积分级数线性度曲线已更新")


if __name__ == "__main__":
    root = tk.Tk()
    app = IntegrationLevelExtractor(root)
    root.mainloop()